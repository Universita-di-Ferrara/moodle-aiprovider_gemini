...
class hook_listener {
    ...
    /**
     * Hook listener for the Gemini action settings form.
     *
    ...
    public static function set_model_form_definition_for_aiprovider_gemini(after_ai_action_settings_form_hook $hook): void {
        ...
        if (isset($mform->_elementIndex['modeltemplate'])) {
            ...
            if ($model == 'custom') {
                ...
                if ($targetmodel) {
                    $endpointelementform = $mform->getElement('endpoint');
                    if ($endpointelementform) {
                        $modelendpoint = $targetmodel->get_endpoint();
                        $endpointelementform->setValue($modelendpoint);
                    }
